...
int ValidaCPF(char cpf[14])
{
    ...
    if(strcmp(cpfNum,"11111111111") != 0 && strcmp(cpfNum,"22222222222") != 0 && strcmp(cpfNum,"33333333333") != 0 && strcmp(cpfNum,"44444444444") != 0 && strcmp(cpfNum,"55555555555") != 0&& strcmp(cpfNum,"66666666666") != 0 && strcmp(cpfNum,"77777777777") != 0 && strcmp(cpfNum,"88888888888") != 0 && strcmp(cpfNum,"99999999999") != 0){
    	for(i = 0; i < TF; i++)
			...
			while(tl<tf && cod>0){
				...
				if(pos<tl){
					...
					if(forn!=0){
						...
						apagar(3);//msg
						//scanf("%f",&preco);
						gotoxy(30,i++),printf("Data de validade [dd mm aa]: ");
	...
	if(tl==0){
		...
		while(cod>0){
			...
			if(pos==tl){
				...
				//gotoxy(30,i++),printf("Estoque: %d -> ",x[pos].est);
				VerifInt(est,i,"Estoque: ",x[pos].est);
				//scanf("%d",&est);
				//gotoxy(30,i++),printf("Preco: R$%.2f -> ",x[pos].preco);
...
void CadastroCliente(TpCliente x[tf],int &tl,int &dados){
	...
	if(tl==tf){
		...
		fflush(stdin);
		gets(cpf);
		while(strlen(cpf)>0&&ValidaCPF(cpf)==-1){
			apagar(3);//msg
					...
					while(tlv<tf && tlvP<tf && cod>0){
						...
						if(pos1<tlp){
							...
							if(pos2<tlvP){
								...
								VerifInt(qtde,i,"Quantidade da venda: ",0);
...
void exclusaoVendas(TpVendas v[tf],TpVendasP vp[tf],TpCliente c[tf],int &tlv,int &tlvP,int tlc,TpProduto x[tf],int tlp){
	...
	if(tlv==0){
		...
		apagar(3);//msg
		gotoxy(3,23),printf("Digite [0] para sair");
		VerifInt(cod,i,"Codigo da Venda: ",0);
		apagar(3);//msg
		while(tlv>0 && cod>0){
	...
	if(TLV > 0)
	{
		while(it<TLV+1)
		{
			...
			for(pos = buscaVendasP2(vp,TLVP,it), tot = 0.0; it == vp[pos].codV; pos++,i++)
			{
				...
				gotoxy(46,i),printf("%d",vp[pos].qtde);
				tot += vp[pos].qtde * vp[pos].preco;
				gotoxy(49,i),printf("R$%.2f",vp[pos].qtde*vp[pos].preco);
				gotoxy(59,i),printf("%s",f[buscaFornecedor(f,TLF,p[buscaProduto(p,TLP,vp[pos].codP)].forn)].nome);
				if(i>19)
					PaginaVendas(i);
			}
